...
import { X, Camera, Upload, Loader2, CheckCircle, AlertCircle, RefreshCw, Pencil } from "lucide-react";
...
type Step = "choose" | "camera" | "preview" | "identifying" | "success" | "unrecognized" | "error";
...
export function ImageSearchModal({ onClose, onIdentified }: ImageSearchModalProps) {
  ...
  const handleIdentify = async () => {
    ...
    try {
      ...
      if (res.status === 422 || (res.ok && !data.productName)) {
        ...
        <div className="p-5">
          ...
          {step === "choose" && (
            <div className="space-y-3">
              <p className="text-sm text-slate-500 mb-4">选择图片来源，AI 会自动识别产品名称</p>
          ...
          {step === "identifying" && (
            <div className="py-8 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <p className="font-semibold text-slate-800">AI 正在识别产品…</p>
                <p className="text-sm text-slate-500 mt-1">通常只需 2–3 秒</p>
              </div>
            </div>
          )}
